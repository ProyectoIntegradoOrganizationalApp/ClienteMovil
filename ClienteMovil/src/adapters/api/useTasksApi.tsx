...
export const useTasksApi = () => {
  ...
  const [data, setData] = useState<Array<Task> | Task>();
  ...
  const createTask = (idapp: string, name: string, description: string) => {
    setLoading(true);
  ...
  const editTask = (
     ...
     */
    const props: RequestParams = {
      url: `${API}/${idapp}/task_app/task/${id}`,
      method: "PUT",
      headers: new AxiosHeaders({
        "Content-Type": "application/json",
        Authorization: `Bearer ${user?._token}`,
  ...
  const deleteTask = (idapp: string, id: string) => {
    setLoading(true);
   ...
   *  @param info
   */
  const handleData = (info: TaskWrapper | TaskDTO | ApiError) => {
    /**
     * Hay Error
     */
     ...
     * Si no hay error
     */
    if (info && "tasks" in info) {
      // Quitamos los errores en caso de que los halla
      setError(undefined);

      // Cambiamos el state
      //console.log(info);
      let tasks: Array<Task> = TaskMapper.prototype.mapArrayTo(info.tasks);
      setData(tasks);
    }
    ...
    if (info && "id" in info) {
      ...
      let task: Task = TaskMapper.prototype.mapTo(info);
      setData(task);
    }
